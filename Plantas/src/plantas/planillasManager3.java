...
public class planillasManager3 extends JFrame{
    ...
    public void planillaMeses(){
        ...
        try (CallableStatement cst = cn.con.prepareCall("{CALL planta3.`Planilla`(?, ?, ?)}")) {
    ...
    void aceptarPlanilla(int idPlanilla) {
        ...
        try {
            ...
            for (int row = 0; row < Table.getRowCount(); row++) {
                ...
                try (PreparedStatement pst = cn.con.prepareStatement("INSERT INTO planta3.lineplanillas (idPlanilla,lineNum, idEmpleado,nombre, hora_entrada, hora_salida, fecha, horas_laboradas, horas_extra, horas_normales, salario_bruto, salario_neto) VALUES (?,?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
                    pst.setInt(1, idPlanilla);
    ...
    void borrarMarca() {
        Conexion cn = new Conexion();
        try {
            try (PreparedStatement pst = cn.con.prepareStatement("DELETE FROM planta3.marcas1 WHERE id_empleado = ? and fecha = ?" )) {
    ...
    void borrarPlanilla(int idPlanilla) {
        Conexion cn = new Conexion();
        try {
            try (PreparedStatement pst = cn.con.prepareStatement("DELETE FROM planta3.planillas WHERE id = ?  ")) {
                pst.setInt(1, idPlanilla);
                pst.executeUpdate();
                PreparedStatement pst2 = cn.con.prepareStatement("DELETE FROM planta3.lineplanillas WHERE idPlanilla = ?  ");
